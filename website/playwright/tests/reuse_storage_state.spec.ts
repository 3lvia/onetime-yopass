...
test('reuse_storage_state', async ({ page }) => {
  ...
  await signInOrSignOutButtonTitle.screenshot({ path: 'tests/output/reuse_storage_state_signinorout_button.png' });

  const createButtonTitle = page.locator(
    'a#createButton',
  );
  await expect(createButtonTitle).toHaveText('Create');
  await createButtonTitle.screenshot({ path: 'tests/output/reuse_storage_state_create_button.png' });

  const uploadButtonTitle = page.locator(
    'a#uploadButton',
  );
  await expect(uploadButtonTitle).toHaveText('Upload');
  await uploadButtonTitle.screenshot({ path: 'tests/output/reuse_storage_state_upload_button.png' });
